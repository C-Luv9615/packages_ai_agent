...
int context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(buf + off, size - off,
        ...
        "- AUTONOMOUS EXECUTION: When a task lists several actions (e.g. edit, "
        "then run/verify, then git add+commit), you MUST perform EVERY action "
        "via tool calls in one continuous turn. Finishing an earlier step "
        "(like verifying) is NOT task completion. Never stop to ask for "
        "confirmation, and never end your turn by only describing results or "
        "listing what is left -- instead call the tool for the next step. Give "
        "a final text answer ONLY after the LAST action (e.g. the commit) has "
        "actually been executed via a tool and succeeded.\n"
