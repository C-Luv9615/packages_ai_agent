...
{
    /* mallinfo() walks the full heap and DEBUGASSERTs on any free-chunk
     * link inconsistency — which reboots the board via recursive assert on
     * this target.  Skip the walker; report a non-iterating conservative
     * value instead.  (The agent_mem.h change has the full rationale.) */
    printf("Heap: mallinfo() disabled on this target to avoid heap-walker "
           "assert reboot. Use 'free' or /proc if available.\n");
}
